...
//uniform vec3 vmdspherepos;       // center of rendered sphere in model coords
//uniform float vmdsphererad;      // radius of rendered sphere in model coords
...
void main(void) {
  ...
  // XXX ATI drivers can't handle this yet, they use fixed-function clipping
  //     and punt to software when this is set
#if 0
// #ifdef __GLSL_CG_STDLIB
  // XXX short-term workaround until NVidia's drivers correctly support
  //     assignments to gl_ClipVertex.  The currently shipping Cg stdlib
  //     doesn't have ClipCoord[] yet, so even this workaround is beyond
  //     what is currently shipping.
  for (int clipplane; clipplane < 6; clipplane++) {
    gl_ClipCoord[clipplane] = dot(ecpos, gl_ClipPlane[clipplane]);
  }
#else
  gl_ClipVertex = ecpos;
#endif
  ...
  vec4 spos = gl_ModelViewMatrix * vec4(0.0, 0.0, 0.0, 1.0);
  ...
  vec4 rspos = gl_ModelViewMatrix * vec4(1.0, 0.0, 0.0, 1.0);
