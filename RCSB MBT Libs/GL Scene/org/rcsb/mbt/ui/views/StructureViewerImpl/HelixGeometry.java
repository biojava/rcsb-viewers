//  $Id: HelixGeometry.java,v 1.1 2007/02/08 02:38:52 jbeaver Exp $
//
...
//  Initial commit
//
//  Revision 1.15  2005/06/30 23:25:37  agramada
//  Removed some debuging code.
//
//  Revision 1.14  2005/06/29 17:49:22  agramada
...
//
//  Revision 1.9  2004/01/29 18:05:27  agramada
//  Removed General Atomics from copyright
//
//  Revision 1.8  2004/01/21 21:56:16  agramada
//  Some minor comment change/improvement.
//
//  Revision 1.7  2003/12/09 00:14:13  agramada
//  Removed some commented material and reference to the debug class.
//
//  Revision 1.6  2003/12/08 22:11:10  agramada
...
//  rendering.
//
//  Revision 1.4  2003/07/17 18:00:40  agramada
//  Added code to allow the use of the line style.
//
//  Revision 1.3  2003/07/14 20:16:48  agramada
//  Set a detach capability of the returned branch.
//
...
import org.rcsb.mbt.ui.views.StructureViewerImpl.Priestle;
...
//	private final float start, end;
	...
	private int ssShape = HelixGeometry.RIBBON;
	...
	public HelixGeometry(final StructureComponent sc) {
		super(sc);
		// helixStyle = CrossSectionStyle.OBLONG;
		// csType = CrossSectionStyle.RECTANGULAR_RIBBON;
		// helixStyle = CrossSectionStyle.DOUBLE_FACE;
		this.csType = CrossSectionStyle.REGULAR_POLYGON;
		// polygonStyle = PolygonAttributes.POLYGON_FILL;
		this.ssShape = HelixGeometry.RIBBON;
	...
	public DisplayLists generateJoglGeometry(final GL gl, final GLU glu, final GLUT glut) {
		...
		if (this.ssShape == HelixGeometry.CYLINDER) {
			this.segments = 2;
		}

		// Create an object for Hermite sampling
		final Hermite hermite = new Hermite();
		hermite.setKnotWeight(HelixGeometry.HELIX_HERMITE_FACTOR);
		...
		if (this.ssShape == HelixGeometry.RIBBON) {
			if (this.csStyle == null) {
				...
				if ((this.facets != this.csStyle.getVertexCount())
						&& ((this.csType == CrossSectionStyle.ROUNDED_TUBE) || (this.csType == CrossSectionStyle.REGULAR_POLYGON))) {
					this.csStyle.setVertexCount(this.facets + 1);
				}
			}
		...
		} else if (this.ssShape == HelixGeometry.CYLINDER) {
...
//		}
	...
	private void getTangentsAndBinormals(final int i, final Vec3f[] coords, final Vec3f vec1,
		...
		if (i == 0) {
			...
			ncvec.scale((float)Math.cos(HelixGeometry.HELIX_BETA));
			nrvec.scale((float)Math.sin(HelixGeometry.HELIX_BETA));
			...
			vec1.value[0] = vvec1.coordinates[0];
			vec1.value[1] = vvec1.coordinates[1];
			vec1.value[2] = vvec1.coordinates[2];
			vec2.value[0] = vvec2.coordinates[0];
			vec2.value[1] = vvec2.coordinates[1];
			vec2.value[2] = vvec2.coordinates[2];

			cvec.scale((float)Math.sin(HelixGeometry.HELIX_ALPHA));
			rvec.scale((float)Math.cos(HelixGeometry.HELIX_ALPHA));
			bin2.add(cvec, rvec);
			if (this.previousCaCoord == null) {
				bin1.set(bin2);
...
//	}
	 ...
	 */
	public void setSsShape(final int ssshape) {
	 ...
	 * ribbon.
	 */
	public int getSsShape(final int ssshape) {
		return this.ssShape;
	}
	...
	private void processQualityInfo(final float quality) {
		...
		if ((this.csType == CrossSectionStyle.REGULAR_POLYGON)
