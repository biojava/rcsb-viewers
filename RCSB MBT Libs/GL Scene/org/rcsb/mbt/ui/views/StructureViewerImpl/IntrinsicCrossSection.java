//  $Id: IntrinsicCrossSection.java,v 1.1 2007/02/08 02:38:52 jbeaver Exp $
//
...
//
//  History:
//  $Log: IntrinsicCrossSection.java,v $
...
//
//  Revision 1.3  2004/01/29 18:05:28  agramada
//  Removed General Atomics from copyright
//
//  Revision 1.2  2003/12/09 00:43:23  agramada
//  Removed reference to debug class.
//
//  Revision 1.1  2003/06/24 22:26:58  agramada
...
import org.rcsb.mbt.model.geometry.Quat4f;
...
public class IntrinsicCrossSection {
	...
	for( int i = 0; i < this.vertex.length; i++ ) {
	    ...
	    vvertex.set( scale.coordinates[0]*vv.coordinates[0], scale.coordinates[1]*vv.coordinates[1], scale.coordinates[2]*vv.coordinates[2] );

	    // Now, we have to convert back to Vec3f format.
	    this.vertex[i].value[0] = vvertex.coordinates[0];
	    this.vertex[i].value[1] = vvertex.coordinates[1];
	    this.vertex[i].value[2] = vvertex.coordinates[2]; 
	...
	x = sinHalfT*vn.coordinates[0];
	y = sinHalfT*vn.coordinates[1];
	z = sinHalfT*vn.coordinates[2];
	...
	for( int i = 0; i < this.vertex.length; i++ ) {
	    vv.set( this.vertex[i].value[0], this.vertex[i].value[1], this.vertex[i].value[2] );
	    vv.sub( vx0 );
	    q.set( vv.coordinates[0], vv.coordinates[1], vv.coordinates[2], 0.0f );
	    p.set( q1 );
	    q.mulInverse( q1 );
	    p.mul( q );

	    vvertex.set( p.coordinates[0], p.coordinates[1], p.coordinates[2] );
	    vvertex.add( vx0 );
	    this.vectorVertex[i].set( vvertex.coordinates[0], vvertex.coordinates[1], vvertex.coordinates[2] );
    ...
    public void setNormalsByVector( final Vec3f direction ) {
      ...
      vd.coordinates[0] = direction.value[0];
      vd.coordinates[1] = direction.value[1];
      vd.coordinates[2] = direction.value[2];
      ...
      for( int i = 0; i < this.vertex.length; i++ ) {
	v1.coordinates[0] = this.vertex[i].value[0];
	v1.coordinates[1] = this.vertex[i].value[1];
	v1.coordinates[2] = this.vertex[i].value[2];

	j = (i+1)%modulo; // Should cycle when i+1 becomes bigger than vertexCount.
	v2.coordinates[0] = this.vertex[j].value[0];
	v2.coordinates[1] = this.vertex[j].value[1];
	v2.coordinates[2] = this.vertex[j].value[2];
	...
	this.normal[i] = new Vec3f();
	this.normal[i].value[0] = vNormal.coordinates[0];
	this.normal[i].value[1] = vNormal.coordinates[1];
	this.normal[i].value[2] = vNormal.coordinates[2];
	this.vectorNormal[i] = new Vector3f( vNormal.coordinates[0], vNormal.coordinates[1], vNormal.coordinates[2] );
      }
    }
  ...
  public void averageNormals() {
    ...
    this.normal[0].value[0] = this.vectorNormal[0].coordinates[0];
    this.normal[0].value[1] = this.vectorNormal[0].coordinates[1];
    this.normal[0].value[2] = this.vectorNormal[0].coordinates[2];
    ...
    for( int i = 1; i < this.vertexCount-1; i++ ) {
      ...
      this.normal[i].value[0] = this.vectorNormal[i].coordinates[0];
      this.normal[i].value[1] = this.vectorNormal[i].coordinates[1];
      this.normal[i].value[2] = this.vectorNormal[i].coordinates[2];
    }
