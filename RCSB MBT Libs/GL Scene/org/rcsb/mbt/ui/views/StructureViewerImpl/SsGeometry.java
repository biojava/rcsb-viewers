...
public class SsGeometry extends GeometryEntity {
	...
	public static DisplayLists[] createSs(final Chain c,
		...
		final int helixCsType = CrossSectionStyle.REGULAR_POLYGON;
		// int helixCsType = CrossSectionStyle.RECTANGULAR_RIBBON;
		final int strandCsType = CrossSectionStyle.RECTANGULAR_RIBBON;
		final int turnCsType = CrossSectionStyle.ROUNDED_TUBE;
		final int coilCsType = CrossSectionStyle.ROUNDED_TUBE;
		...
		return SsGeometry.createSs(c, structureMap, styles, ribbon,
				...
				coilCsType, HelixGeometry.RIBBON, gl, glu, glut);
	}
	...
	public static DisplayLists[] createSs(final Chain c,
		...
		final int helixCsType = CrossSectionStyle.REGULAR_POLYGON;
		// int helixCsType = CrossSectionStyle.ROUNDED_TUBE;
		final int strandCsType = CrossSectionStyle.RECTANGULAR_RIBBON;
		final int turnCsType = CrossSectionStyle.ROUNDED_TUBE;
		final int coilCsType = CrossSectionStyle.ROUNDED_TUBE;
	...
	public static DisplayLists[] createSs(final Chain c,
			final StructureMap structureMap, final StructureStyles styles, final boolean ribbon,
			final int helixSmoothingSteps, final int strandSmoothingSteps,
			final int turnSmoothingSteps, final int coilSmoothingSteps, final int helixSsShape, final GL gl, final GLU glu, final GLUT glut)
			throws FragmentTooShortException {
		// Quality factors
		final float helixQuality = 0.8f;
		final float coilQuality = 0.8f;
	...
	public static DisplayLists[] createSs(final Chain c,
			...
			final int coilSmoothingSteps, final int helixCsType, final int strandCsType,
			final int turnCsType, final int coilCsType, final int helixSsShape, final GL gl, final GLU glu, final GLUT glut)
			throws FragmentTooShortException {
		...
		boolean hasLeading;
		boolean hasTrailing;
		String conformationType, previousConformationType, nextConformationType;
		float[][] colorMap = null;
		...
		final CrossSectionStyle helixCsStyle = new CrossSectionStyle(
				CrossSectionStyle.REGULAR_POLYGON, diams);
		final CrossSectionStyle uniformCsStyle = new CrossSectionStyle(
				CrossSectionStyle.RECTANGULAR_RIBBON, diams);
		...
		for (int fIndex = 0; fIndex < fragmentCount; fIndex++) {
			...
			if (fIndex > 0) {
				...
				previousConformationType = Conformation.TYPE_UNDEFINED;
			}
			if (fIndex < fragmentCount - 1) {
				nextConformationType = chainItem.getFragmentType(fIndex + 1);
			} else {
				nextConformationType = Conformation.TYPE_UNDEFINED;
			}
			...
			trailingResidueIndex = endResidueIndex = fragment
					.getEndResidueIndex();
			if ((fIndex > 0)
					&& (chainItem.getFragmentType(fIndex - 1) != Conformation.TYPE_UNDEFINED)) {
			...
			if (conformationType == Conformation.TYPE_UNDEFINED) {
				...
				//
			} else if (conformationType == "NONE") {
				// System.err.println( "NONE " + conformationType );
			} else if (conformationType == StructureComponentRegistry.TYPE_COIL) {
				startIndex = 0;
				hasLeading = false;
				hasTrailing = false;
				cGeom = new org.rcsb.mbt.ui.views.StructureViewerImpl.CoilGeometry(fragment);
				cGeom.setPriestleSteps(coilSmoothingSteps);
				cGeom.setQuality(coilQuality);
				cGeom.setUserData(fragment);
				if (!ribbon) {
					cGeom.setCrossSectionType(CrossSectionStyle.POINT);
				} else {
				...
				if ((leadingResidueIndex < startResidueIndex)
						&& ((previousConformationType != "NONE") && (!gapPrecedes))
						&& (previousConformationType != StructureComponentRegistry.TYPE_COIL)) {
				...
				if ((trailingResidueIndex > endResidueIndex)
						&& (nextConformationType != "NONE" && (!gapFollows))
			...
			} else if ((conformationType == StructureComponentRegistry.TYPE_TURN)
					| ((conformationType == StructureComponentRegistry.TYPE_HELIX) & (residueCount < 3))) {
				startIndex = 0;
				hasLeading = false;
				hasTrailing = false;
				tGeom = new org.rcsb.mbt.ui.views.StructureViewerImpl.CoilGeometry(fragment);
				tGeom.setPriestleSteps(turnSmoothingSteps);
				tGeom.setQuality(turnQuality);
				tGeom.setUserData(fragment);
				if (!ribbon) {
					tGeom.setCrossSectionType(CrossSectionStyle.POINT);
				} else {
				...
				/*
				 * if( conformationType == StructureComponentRegistry.TYPE_TURN) {
				...
				if (leadingResidueIndex < startResidueIndex && (!gapPrecedes)) {
					if ((previousConformationType != StructureComponentRegistry.TYPE_COIL)
							& (previousConformationType != StructureComponentRegistry.TYPE_TURN)) {
						residue = chainItem.getResidue(leadingResidueIndex);
						leadingCaAtom = residue.getAlphaAtom();
						hasLeading = true;
						allResidueCount++;
				...
				if (trailingResidueIndex > endResidueIndex && (!gapFollows)) {
					if (chainItem.getFragmentType(fIndex + 1) != StructureComponentRegistry.TYPE_COIL) {
						residue = chainItem.getResidue(trailingResidueIndex);
						trailingCaAtom = residue.getAlphaAtom();
						hasTrailing = true;
						allResidueCount++;
				...
				if (((conformationType == StructureComponentRegistry.TYPE_TURN) & (allResidueCount < 2))
						| ((conformationType == StructureComponentRegistry.TYPE_HELIX) & (allResidueCount < 3))) {
					manageable = false;
			...
			} else if (conformationType == StructureComponentRegistry.TYPE_STRAND) {
				//if(true)
				//continue;
				...
				hasTrailing = false;
				manageable = true;
				sGeom = new org.rcsb.mbt.ui.views.StructureViewerImpl.StrandGeometry(fragment);
				...
				if (!ribbon) {
					sGeom.setCrossSectionType(CrossSectionStyle.POINT);
				} else {
				...
				if ((nextConformationType == StructureComponentRegistry.TYPE_STRAND)
						|| (nextConformationType == StructureComponentRegistry.TYPE_HELIX)
						&& (!gapFollows)) {
					residue = chainItem.getResidue(trailingResidueIndex);
					trailingCaAtom = residue.getAlphaAtom();
					hasTrailing = true;
			...
			} else if (conformationType == StructureComponentRegistry.TYPE_HELIX) {
				//if(true)
				//continue;
				
				hGeom = new org.rcsb.mbt.ui.views.StructureViewerImpl.HelixGeometry(fragment);
				...
				if (!ribbon) {
					hGeom.setCrossSectionType(CrossSectionStyle.POINT);
				} else {
	 ...
	 */
	public void setCrossSectionType(final int type) {
		this.csType = type;
	}
