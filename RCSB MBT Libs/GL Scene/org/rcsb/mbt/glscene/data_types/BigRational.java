package org.rcsb.mbt.glscene.data_types;
...
** @version @(#)$Id: BigRational.java,v 1.1 2007/02/08 02:38:52 jbeaver Exp $
**
...
//	implements Cloneable
{
	...
	*/
	...
	{
		if (BigRational.bigIntegerZerop(q)) {
	...
	*/
	...
	{
		this(n, BigRational.BIG_INTEGER_ONE);
	}
	...
	*/
	...
	{
		this(BigRational.bigIntegerValueOf(n), BigRational.bigIntegerValueOf(q));
	}
	...
	*/
	...
	{
		this(BigRational.bigIntegerValueOf(n), BigRational.BIG_INTEGER_ONE);
	}
	...
	*/
	public BigRational(final String s, final int radix)
	{
		...
		if (slash != -1) {
			...
			BigRational.checkNumberFormat(d);
			...
			// check for multiple signs or embedded signs
			BigRational.checkNumberFormat(q);
			...
			this.m_n = BigRational.newBigInteger(d, radix);
			this.m_q = BigRational.newBigInteger(q, radix);
			...
			// check for multiple signs or embedded signs
			BigRational.checkNumberFormat(i);
			...
			// check for signs
			BigRational.checkFractionFormat(f);
			...
			final BigInteger iValue = BigRational.newBigInteger(i, radix);
			final BigInteger fValue = BigRational.newBigInteger(f, radix);

			final int scale = f.length();
			BigRational.checkRadix(radix);
			final BigInteger fq = BigRational.bigIntegerPow(BigRational.bigIntegerValueOf(radix), scale);

			this.m_n = BigRational.bigIntegerMultiply(iValue, fq).add(fValue);
			...
			this.m_n = BigRational.newBigInteger(i, radix);
			this.m_q = BigRational.BIG_INTEGER_ONE;
		}
	...
	*/
	...
	{
		this(s, BigRational.DEFAULT_RADIX);
	}

	/** Construct a BigRational from an unscaled value by scaling.
	*/
	public BigRational(final BigInteger unscaledValue, final int scale_, final int radix)
	{
		int scale = scale_;
		...
		BigRational.checkRadix(radix);
		final BigInteger scaleValue = BigRational.bigIntegerPow(BigRational.bigIntegerValueOf(radix), scale);

		if (!negt) {
			this.m_n = unscaledValue;
			this.m_q = scaleValue;
		} else {
			this.m_n = BigRational.bigIntegerMultiply(unscaledValue, scaleValue);
			this.m_q = BigRational.BIG_INTEGER_ONE;
		}
	...
	*/
	...
	{
		this(unscaledValue, scale, BigRational.DEFAULT_RADIX);
	}
	...
	*/
	...
	{
		this(BigRational.bigIntegerValueOf(unscaledValue), scale, radix);
	}
	...
	*/
	private void normalize()
	{
		...
		int ns = this.m_n.signum();
		int qs = this.m_q.signum();
		...
		if (ns == 0 && qs == 0) {
			// [both for speed]
			this.m_n = BigRational.BIG_INTEGER_ZERO;
			this.m_q = BigRational.BIG_INTEGER_ZERO;
			return;
		}
		...
		if (qs == 0) {
			this.m_n = BigRational.BIG_INTEGER_ONE;
			// [for speed]
			this.m_q = BigRational.BIG_INTEGER_ZERO;
			return;
		}
		...
		if (this.m_q == BigRational.BIG_INTEGER_ONE) {
			// [for speed]
			this.m_n = BigRational.proxyBigInteger(this.m_n);
			return;
		}

		// check the symmetric case too, for speed.
		// note: this only covers the normalized-for-speed 1-case.
		if ((this.m_n == BigRational.BIG_INTEGER_ONE || this.m_n == BigRational.BIG_INTEGER_MINUS_ONE) && qs > 0) {
			// [for speed]
			this.m_q = BigRational.proxyBigInteger(this.m_q);
			return;
		}

		// setup torn apart for speed
		BigInteger na = this.m_n;
		BigInteger qa = this.m_q;
		...
		if (!BigRational.bigIntegerOnep(g)) {
			this.m_n = this.m_n.divide(g);
			this.m_q = this.m_q.divide(g);
		}
	...
	*/
	private static BigInteger bigIntegerValueOf(final long n)
	{
		...
		case 0 :
			return BigRational.BIG_INTEGER_ZERO;

		case 1 :
			return BigRational.BIG_INTEGER_ONE;

		case -1 :
			return BigRational.BIG_INTEGER_MINUS_ONE;

		case 2 :
			return BigRational.BIG_INTEGER_TWO;

		case -2 :
			return BigRational.BIG_INTEGER_MINUS_TWO;

		case 10 :
			return BigRational.BIG_INTEGER_TEN;

		case 16 :
			return BigRational.BIG_INTEGER_SIXTEEN;
	...
	* Speeds up comparisions by using constants.
	*/
	private static BigInteger proxyBigInteger(final BigInteger n)
	{
		...
		if (n == BigRational.BIG_INTEGER_ONE) {
			return n;
		}

		if (n == BigRational.BIG_INTEGER_ZERO) {
			return n;
		}

		if (n == BigRational.BIG_INTEGER_MINUS_ONE) {
			return n;
		}

		// more expensive tests later.

		if (n.equals(BigRational.BIG_INTEGER_ONE)) {
			return BigRational.BIG_INTEGER_ONE;
		}

		if (n.equals(BigRational.BIG_INTEGER_ZERO)) {
			// [typically not reached from normalize().]
			return BigRational.BIG_INTEGER_ZERO;
		}

		if (n.equals(BigRational.BIG_INTEGER_MINUS_ONE)) {
			return BigRational.BIG_INTEGER_MINUS_ONE;
		}
	...
	* Speeds up comparisions by using constants.
	*/
	private static BigInteger newBigInteger(final String s, final int radix)
	{
		// note: mind the radix.
	...
	*/
	private static boolean bigIntegerZerop(final BigInteger n)
	{
		// first test is for speed.
		if (n == BigRational.BIG_INTEGER_ZERO) {
			return true;
	...
	* For convenience and speed.
	*/
	private static boolean bigIntegerOnep(final BigInteger n)
	{
		// first test is for speed.
		if (n == BigRational.BIG_INTEGER_ONE) {
			return true;
		...
		return BigRational.bigIntegerEquals(n, BigRational.BIG_INTEGER_ONE);
	}
	...
	*/
	private static BigInteger bigIntegerMultiply(final BigInteger n, final BigInteger m)
	{
		...
		if (BigRational.bigIntegerZerop(n) || BigRational.bigIntegerZerop(m)) {
			return BigRational.BIG_INTEGER_ZERO;
		}

		// optimization: second operand is one (i.e. neutral element).
		if (BigRational.bigIntegerOnep(m)) {
			return n;
		}

		// optimization: first operand is one (i.e. neutral element).
		if (BigRational.bigIntegerOnep(n)) {
			return m;
		}
	...
	* For speed.
	*/
	private static BigInteger bigIntegerPow(final BigInteger n, final int exponent)
	{
		// jump table, for speed.
		switch (exponent) {

		case 0 :
			if (BigRational.bigIntegerZerop(n)) {
	...
	*/
	...
	{
		return BigRational.bigIntegerOnep(this.m_q);
	}
	...
	*/
	...
	{
		final String s = this.m_n.toString(radix);

		if (this.integerp()) {
			return s;
		}

		final String t = this.m_q.toString(radix);
	...
	*/
	...
	{
		return this.toString(BigRational.DEFAULT_RADIX);
	}

	/** Dot-format "[-]i.f" string representation, with a precision.
	* Precision may be negative.
	*/
	public String toStringDot(int precision, final int radix)
	{
		BigRational.checkRadix(radix);
		BigRational scaleValue = new BigRational(
			BigRational.bigIntegerPow(BigRational.bigIntegerValueOf(radix),
		...
		// default round mode.
		BigRational n = this.multiply(scaleValue).round();
		final boolean negt = n.negativep();
		if (negt) {
			n = n.negate();
		}
	...
	*/
	...
	{
		return this.toStringDot(precision, BigRational.DEFAULT_RADIX);
	}
	...
	*/
	public BigRational add(final BigRational that)
	{
		...
		if (BigRational.bigIntegerEquals(this.m_q, that.m_q)) {
			return new BigRational(
				this.m_n.add(that.m_n),
				this.m_q);
		}
		...
		if (that.integerp()) {
			return new BigRational(
				this.m_n.add(that.m_n.multiply(this.m_q)),
				this.m_q);
		}
		...
		if (this.integerp()) {
			return new BigRational(
				this.m_n.multiply(that.m_q).add(that.m_n),
				that.m_q);
		}

		// default case.  [this would handle all cases.]
		return new BigRational(
			this.m_n.multiply(that.m_q).add(that.m_n.multiply(this.m_q)),
			this.m_q.multiply(that.m_q));
	}
	...
	*/
	...
	{
		return this.add(new BigRational(that));
	}
	...
	*/
	public BigRational subtract(final BigRational that)
	{
		...
		if (that.integerp()) {
			return new BigRational(
				this.m_n.subtract(that.m_n.multiply(this.m_q)),
				this.m_q);
		}
		...
		if (this.integerp()) {
			return new BigRational(
				this.m_n.multiply(that.m_q).subtract(that.m_n),
				that.m_q);
		}

		// default case.  [this would handle all cases.]
		return new BigRational(
			this.m_n.multiply(that.m_q).subtract(that.m_n.multiply(this.m_q)),
			this.m_q.multiply(that.m_q));
	}
	...
	*/
	...
	{
		return this.subtract(new BigRational(that));
	}
	...
	*/
	...
	{
		final BigInteger n = BigRational.bigIntegerMultiply(this.m_n, that.m_n);
		...
		final BigInteger q = BigRational.bigIntegerMultiply(this.m_q, that.m_q);
	...
	*/
	...
	{
		return this.multiply(new BigRational(that));
	}
	...
	*/
	public BigRational divide(final BigRational that)
	{
		...
		return new BigRational(
			BigRational.bigIntegerMultiply(this.m_n, that.m_q),
			BigRational.bigIntegerMultiply(this.m_q, that.m_n));
	}
	...
	*/
	...
	{
		return this.divide(new BigRational(that));
	}
	...
	*/
	public BigRational power(final int exponent_)
	{
		int exponent = exponent_;
		final boolean z = this.zerop();
		...
		final BigInteger d = BigRational.bigIntegerPow(this.m_n, exponent);
		final BigInteger q = BigRational.bigIntegerPow(this.m_q, exponent);
	...
	*/
	...
	{
		return this.remainder(new BigRational(that));
	}
	...
	*/
	...
	{
		return this.modulus(new BigRational(that));
	}
	...
	*/
	private BigRational remainderOrModulusAbsolute(final BigRational that)
	{
		final int s = this.signum();
		final int ts = that.signum();
		...
		final BigInteger n = BigRational.bigIntegerMultiply(this.m_n, that.m_q);
		final BigInteger q = BigRational.bigIntegerMultiply(this.m_q, that.m_n);

		final BigInteger r = n.remainder(q);
		final BigInteger rq = BigRational.bigIntegerMultiply(this.m_q, that.m_q);
	...
	*/
	public BigRational absolute()
	{
		...
		return new BigRational(this.m_n.negate(), this.m_q);
	}
	...
	*/
	public BigRational invert()
	{
		...
		// optimization
		if (this == BigRational.ONE || this == BigRational.MINUS_ONE) {
			return this;
		}
		...
		return new BigRational(this.m_q, this.m_n);
	}
	...
	*/
	...
	{
		return this.minimum(new BigRational(that));
	}
	...
	*/
	...
	{
		return this.maximum(new BigRational(that));
	}
	...
	*/
	public int hashCode()
	{
		final int dh = this.m_n.hashCode(), qh = this.m_q.hashCode();
		// dh * qh;
		final int h = ((dh + 1) * (qh + 2));
		return h;
	}
	...
	*/
	public int compareTo(final BigRational that)
	{
		...
		return BigRational.bigIntegerMultiply(this.m_n, that.m_q).compareTo(
			BigRational.bigIntegerMultiply(that.m_n, this.m_q));
	}
	...
	*/
	...
	{
		return this.compareTo(new BigRational(that));
	}
	...
	*/
	public int compareTo(final Object object)
	{
		if (object instanceof Integer) {
			return this.compareTo(((Integer)object).longValue());
		}

		if (object instanceof Long) {
			return this.compareTo(((Long)object).longValue());
		}
		...
		return this.compareTo((BigRational)object);
	}
	...
	*/
	public final static int DEFAULT_ROUND_MODE = BigRational.ROUND_HALF_UP;
	...
	*/
	public BigRational round(final int roundMode)
	{
		...
		return new BigRational(this.roundToBigInteger(roundMode));
	}
	...
	*/
	private BigInteger roundToBigInteger(final int roundMode)
	{
		...
		BigInteger d = this.m_n;
		final BigInteger q = this.m_q;

		final int sgn = d.signum();
		if (sgn == 0) {
			return d;
		}
		...
		boolean up = false;
		final int comp = r.multiply(BigRational.BIG_INTEGER_TWO).compareTo(q);
		...
		case ROUND_HALF_EVEN :
			up = (comp != 0 ?
				comp > 0 :
				!BigRational.bigIntegerZerop(dv.remainder(BigRational.BIG_INTEGER_TWO)));
			break;
		...
		if (up) {
			dv = dv.add(BigRational.BIG_INTEGER_ONE);
		}
	...
	*/
	...
	{
		return this.round(BigRational.DEFAULT_ROUND_MODE);
	}
	...
	*/
	...
	{
		return this.round(BigRational.ROUND_FLOOR);
	}
	...
	*/
	...
	{
		return this.round(BigRational.ROUND_CEILING);
	}
	...
	*/
	...
	{
		return this.round(BigRational.ROUND_DOWN);
	}
	...
	*/
	...
	{
		final BigRational ip = this.integerPart();
		final BigRational fp = this.subtract(ip);
...
*/
	...
	// **JB may be inefficient, and is only as accurate as a double.
	public BigRational sqrt() {
		return new BigRational("" + Math.sqrt(this.m_n.longValue() / this.m_q.longValue()));
	}
	
//	 **JB may be inefficient, and is only as accurate as a double.
	public BigRational acos() {
		return new BigRational("" + Math.acos(this.m_n.longValue() / this.m_q.longValue()));
	}
