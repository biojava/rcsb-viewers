...
//  Initial commit
//
//  Revision 1.4  2004/10/27 20:05:42  moreland
//  Now correctly returns sumOfSquares instead of Math.acos( sumOfSquares ).
//
//  Revision 1.3  2004/10/22 23:33:28  moreland
//  Added a number of vector algebra methods including crossProduct, dotProduct,
//  normalizeVector, vectorLength, angleAxisRotate, matrixRotate,
//  and rotationToMatrix.
//
//  Revision 1.2  2004/04/09 00:15:20  moreland
//  Updated copyright to new UCSD wording.
//
//  Revision 1.1  2004/02/05 18:33:31  moreland
//  First version.
//
//  Revision 1.0  2004/02/04 18:06:54  moreland
//  First version.
//
...
import org.rcsb.mbt.model.util.BondFactory;


/**
 *  This class provides a number of methods for performing linear algebra
 *  calculations on cartesian coordinate arrays.
 *  <P>
...
public class Algebra
{
	...
	public static float angle( final double _v1[] , final double _v2[], final double _v3[])
	{
		...
		return angle((float[])v1, (float[])v2, (float[])v3);
	}
	...
	public static void normalizeVector( final float[] inVector, final float[] outVector )
	{
		...
		float length = Algebra.vectorLength( inVector );
		if ( length == 0.0 ) {
			length = 1.0f;
		}
	...
	public static void normalizeVector( final double[] inVector, final double[] outVector )
	{
		...
		double length = Algebra.vectorLength( inVector );
		if ( length == 0.0 ) {
			length = 1.0;
		}
		for ( int i=0; i<inVector.length; i++ ) {
	...
	{
		Algebra.normalizeVector( vector, vector );
	}
	...
	public static void angleAxisRotate( final float[] angleAxis, final float[] point )
	{
		...
		Algebra.rotationToMatrix( angleAxis, matrix );
		Algebra.matrixRotate( matrix, point );
	}
