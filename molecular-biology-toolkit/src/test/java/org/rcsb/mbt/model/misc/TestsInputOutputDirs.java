...
{
	static public final String projectDirName = "molecular-biology-toolkit";
	...
	public TestsInputOutputDirs(String invocationClassName, boolean doesFileCompares) throws IOException
	{
		...
		File topTestInputDir = new File(projectDir.getAbsolutePath() + sepChar + "src/test/resources/"+ testInputTopDirName);
		if (!topTestInputDir.exists())
			throw new IOException("Can't find top level test directory. " + topTestInputDir );
		 ...
		 try
	 	 {
			 String fullOutputPath = projectDir.getAbsolutePath() + sepChar + "target/" + testOutputTopDirName +
